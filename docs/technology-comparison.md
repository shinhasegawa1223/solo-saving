...
### 目的: 「個人開発で最速かつ保守しやすい」構成

本プロジェクトは個人開発であり、以下を重視しました。

1. **開発速度**: 限られた時間でMVPを完成させる必要がある
2. **学習コスト**: 既存スキルを活かしつつ、新しい技術も適度に取り入れる
3. **保守性**: 半年後の自分が見ても理解できるコード構造
4. **金融計算への適性**: 株価取得、為替計算、チャート描画に強いエコシステム

### Next.js × FastAPI を選んだ理由
- **フロントとバックの分離**: 責務を明確に分けることで、片方の変更がもう片方に影響しにくい
- **Pythonの金融ライブラリ活用**: yfinance, pandas, numpy が株価取得・計算に非常に便利。JavaScriptでは代替が少ない
- **型安全性の両立**: TypeScript (Next.js) + Pydantic (FastAPI) で両側とも型チェックが効く

### Biome + Ruff を選んだ理由
- **高速**: ESLint + Prettier より10〜20倍速く、CI/CDの待ち時間を削減
- **統一設定**: 複数ツールの設定ファイル管理が不要で、シンプル
- **Rust製**: 依存関係が少なく、環境構築トラブルが減る

### Bunを選んだ理由
- **npm互換の高速パッケージマネージャー**: yarn/pnpmより更に高速
- **Next.jsとの相性**: Viteは使えないが、Bunはランタイムとして利用可能
- **将来性**: 急速に成熟しており、Node.jsの置き換え候補として注目
