...
### PostgreSQLを選択した理由
- **時系列データへの強さ**: 株価履歴や資産推移など、時系列データの分析クエリ（ウィンドウ関数、CTE）に強い。
- **データ整合性**: 金融データは「数字が合わない」ことが致命的。ACIDトランザクションで整合性を強固に担保。
- **拡張性**: 将来的にTimescaleDB（時系列拡張）を追加する選択肢も残しておける。
- **無料**: 個人開発でランニングコストを抑えたい。

### シングルユーザー前提で設計した理由
- **YAGNI原則**: 現時点で「複数ユーザー対応」は必要ない。存在しない要件に備えた設計は過剰。
- **シンプルさ優先**: user_idカラムやRLSを省略することで、開発速度とクエリのシンプルさを確保。
- **将来対応は容易**: 必要になった時点で `user_id` を追加するマイグレーションを行えばよい。

### UUIDを主キーに採用した理由
- **分散環境対応**: 将来的に複数サービスからデータを作成する可能性を考慮。連番だと衝突の恐れあり。
- **セキュリティ**: URLに `/assets/123` のような連番IDを露出すると、他のデータが推測されやすい。UUIDなら推測困難。
- **パフォーマンス**: 5年運用で2,000行程度を想定しており、UUIDのインデックスパフォーマンスは問題にならない規模。

### asset_snapshots（スナップショット）テーブルを設けた理由
- **クエリ効率**: 「過去30日の総資産推移」を毎回リアルタイム計算するのは非効率。事前集計データを持つことで高速表示。
- **一貫性**: スナップショット時点の為替レートで評価額を固定できる。後から為替が変動しても過去の数字は変わらない。

### 正規化レベルについて
- **第3正規化を基本**: 重複データを排除し、更新時の不整合を防止。
- **ただしスナップショットは非正規化**: 日次集計値をそのまま保存することで、読み取り性能を優先（レポート系テーブルの常套手段）。
