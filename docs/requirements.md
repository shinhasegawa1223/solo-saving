...
### なぜこのアプリを作るのか？（目的）
- **既存サービスへの不満**: 証券会社のアプリは複数口座を横断できない。家計簿アプリは資産管理に特化していない。
- **自分だけのダッシュボード**: 必要な情報だけを、見たいレイアウトで表示したい。
- **学習目的**: フルスタック開発（Next.js + FastAPI + AWS）の実践経験を積みたい。

### なぜWebアプリか？（技術選定の背景）
- **PC利用が前提**: 資産管理は腰を据えて行う作業。スマホアプリより PCブラウザが適している。
- **デプロイの容易さ**: モバイルアプリはストア審査が必要。Webなら即デプロイ可能。
- **クロスプラットフォーム**: MacでもWindowsでも同じ体験を提供できる。
